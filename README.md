...
| SAS log | `execute_sas_code`, `get_job_log`, `submit_batch_job` | Colour-coded `ERROR` / `WARNING` / `NOTE` lines, jump to the next problem, problems-only and hide-notes filters, the listing on its own tab, and SAS's own HTML output on a Results tab (see [HTML results](#html-results)). A submitted batch job is polled from the view until it ends, then its log is fetched. |
...
Everything about this is additive. The tool's return value is unchanged, so a client without the extension — Claude Code, a script, an older client — sees exactly what it saw before. Every request a view makes travels through the host to this server as an ordinary tool call with the session's token; the browser never talks to Viya, and a tool a deployment withholds through `MCP_TIERS` or `MCP_READ_ONLY` has no view either — nor does any control inside a view that would have called it, so a read-only deployment shows the glossary browser without its Publish button rather than with one that can only fail. The pages are fully inlined — the vendored `@modelcontextprotocol/ext-apps` bridge included — so they render on hosts that ignore CSP declarations and on an air-gapped Viya. `MCP_APPS=false` registers no views.

Whether a view is rendered depends on the client and on how it is connected:

| Client | Views |
|---|---|
| ChatGPT, Microsoft 365 Copilot, VS Code with GitHub Copilot, Cursor, Goose | Rendered — these clients implement the extension. |
| Claude (web, Desktop, Cowork) connected to the HTTP server as a **custom connector** | Rendered, including the SAS log view's Results tab. The connector speaks the 2026 protocol revision, whose handshake has no session, so the server needs `MCP_STATELESS_HTTP=true` and a URL that Anthropic's servers can reach. |
| Claude Desktop with the server in `claude_desktop_config.json` (which accepts stdio servers only) | **Not rendered since Claude Desktop 2.2553 (22 September 2026).** Desktop passes a local server's tools to the chat through a bridge that rebuilds each tool's metadata and drops the view binding, so the chat never asks for the page. The tools work as before; results are text, and `execute_sas_code` still returns its HTML results link. Reported as [anthropics/claude-ai-mcp#1069](https://github.com/anthropics/claude-ai-mcp/issues/1069). |
| Claude Code | Not rendered — text only, by design. |

A view's own controls call other tools (the Results tab reads the page through `download_file`, Tier 2), so a deployment that withholds those tiers gets the view without that control. View URIs carry a fingerprint of the page (`ui://sas-viya/sas-log/execute_sas_code.<hash>.html`) that changes with the server version, the page files and the tier selection, because hosts cache a page by its URI — claude.ai even across a connector being removed and added again; the plain URI is served too, for hosts holding an older tool listing.

### HTML results

`execute_sas_code` returns the SAS log and listing as text, which is what the model reads. By default it also produces the page SAS Data and AI Studio would show, with tables styled by ODS and graphs drawn, and returns a link to it:

- **How.** The compute context opens no HTML destination of its own, so the server runs the code between an `ods html5` statement and its close. Before the close comes `;*';*";*/;run;quit;`, which ends whatever the code left open (an unterminated statement, quote or comment, or a step without `run;`) so that the close still runs. None of this appears in the log the model reads. `html_results=false` submits the code exactly as given.
- **The link.** When the code printed something, the page is saved to the Viya Files service and the result gains `html_results_url`. Opening it in a browser asks you to sign in to SAS Viya; the file sits in no folder, so Viya's default rules let only the person who ran the code (and administrators) open it. Pages are saved without an expiration, the Files service's default, so they stay until someone deletes them.
- **In the SAS log view**, the page is the Results tab, shown first when the code ran cleanly. The view fetches it through `download_file`, so a deployment without Tier 2 gets the link but not the tab.
- **Cost.** Opening and closing the destination adds about a tenth of a second to a job, and saving the page is one more request. The page itself, 40–70 KB and mostly ODS's stylesheet, never enters the model's context; only the link does.
...
*SAS Data Governance (metadata discovery & profiling):*
...
- **execute_sas_code**: Execute SAS code snippets and retrieve execution results (log and listing output), plus a link to the [HTML results](#html-results) unless `html_results=false`. Runs in a reusable compute session that is kept warm across calls, so SAS state (WORK tables, macro variables, assigned librefs) persists between successive calls
...
discovery agent named 'Public'"* on instances where SAS Data Governance has no discovery
